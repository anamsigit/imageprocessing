...
#ARDUINO
import serial
# arduino = serial.Serial(port='COM5', baudrate=115200, timeout=.1) #baudrate adalah saluran mana yang digunakan, umumnya saya menggunakan baudrate 115200
arduino = serial.Serial(port='COM4', baudrate=250000, timeout=.1) #baudrate adalah saluran mana yang digunakan, umumnya saya menggunakan baudrate 115200
'''
Secara praktisi, meningkatkan nilai baudrate akan semakin meminimalisir kesalahan / nilai 0 yang dikirim. Itu dibuktikan ketika menggunakan baudrate 250000 sangat minim terjadi pengiriman nilai 0, dan ketika anda menggunakan baudrate yang lebih rendah misalnya 9600 akan banyak angka 0 yang dikirim, akan selalu di derajat 0 ketika anda menggunakan baudrate 600
...
while True:
	...
	# color space
	lebarlayar = 600
	frame = imutils.resize(frame, width=lebarlayar)
	...
	if len(cnts) > 0 and len(cnts2) > 0:
		...
		if radius > 10:
			...
			# print(updatednilaix)
			# print(lebarlayar)
			if updatednilaix >= lebarlayar/2:
				updatednilaix = updatednilaix - (lebarlayar/2)
				servo = (60*updatednilaix)/(lebarlayar/2)
				sudutservo = int(90+servo)
			elif updatednilaix < lebarlayar/2:
				updatednilaix = (lebarlayar/2) - updatednilaix
				servo = (60*updatednilaix)/(lebarlayar/2)
				sudutservo = int(90-servo)
			strsudutservo=str(sudutservo)
			print(strsudutservo)
			logPIDpoint.append(strsudutservo)
			arduino.write(bytes(str.encode(strsudutservo)))
